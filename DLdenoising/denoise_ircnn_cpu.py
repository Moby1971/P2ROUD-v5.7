...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='ircnn_gray', help='ircnn_gray | ircnn_color')
    parser.add_argument('--testset_name', type=str, default='esaote_2_5x_few', help='test set | bsd68 | set12')
    parser.add_argument('--need_degradation', type=bool, default=False, help='add noise or not')
    parser.add_argument('--noise_level_img', type=int, default=5, help='noise level: 5, 10, 15, 25, 50')
    ...
    current_idx = min(24, int(np.ceil(args.noise_level_img/2)-1)) # current_idx+1 th denoiser


    args.task_current = 'dn'       # fixed, 'dn' for denoising | 'sr' for super-resolution
    args.sf = 1                    # unused for denoising
    ...
    # load model
    # ----------------------------------------
    model25 = torch.load(model_path, weights_only=True)
